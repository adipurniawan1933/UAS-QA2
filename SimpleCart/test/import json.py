...
def test_post_cart(client):
    ...
    # Assertion untuk kode status HTTP 200
    assert response.status_code == 200, f"Failed to add item to cart. Status code: {response.status_code}"

    # Assertion untuk memastikan pesan "data created" ada dalam respons
    response_data = response.json()
    assert "data created" in response_data, "Expected 'data created' message not found in response"
